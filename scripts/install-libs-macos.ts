...
async function main(){
    ...
    await downloadFile("https://github.com/alvindimas05/Fluyer-API/releases/latest/download/macos-libs.zip", zipPath);
    await extractZip(zipPath, destPath);
    await fs.symlink(path.resolve(destPath, "libmpv.2.dylib"), path.resolve(destPath, "libmpv.dylib"))
}
