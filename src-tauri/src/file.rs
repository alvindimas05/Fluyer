...
pub fn get_all_music() -> Option<Vec<MusicMetadata>> {
    ...
    for music_path in &musics {
        ...
        // Check if exists
        let mut stmt = conn.prepare("SELECT modified_at FROM musics WHERE path = ?1").ok()?;
            ...
            Err(rusqlite::Error::QueryReturnedNoRows) => {
                ...
                let res = conn.execute(
                    "INSERT INTO musics (
                        ...
                        track_number, genre, bits_per_sample, sample_rate, modified_at
    ...
    let mut stmt = conn.prepare("SELECT path, duration, title, artist, album, album_artist, track_number, genre, bits_per_sample, sample_rate FROM musics").ok()?;
