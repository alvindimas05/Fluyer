<script lang="ts">
	...
	async function setCurrentVisualizer(index: number) {
		...
		View.data.renderVisualization = visualizers[currentVisualizerIndex].render.bind(
			visualizers[currentVisualizerIndex]
		);
	}
...
<svelte:window onresize={View.onWindowResize} onkeydown={onKeyDown} />
<div class="fixed z-[-2] h-full w-full bg-black"></div>
<div class="fixed z-[-1] h-full w-full" bind:this={container}></div>
<div
	class="show-then-fade ms-3 w-fit font-light"
	style="margin-top: {marginTop}px;"
	use:showThenFade
>
	<p class="text-3xl text-gray-300">Vissonance</p>
	...
	<p class="mt-2 cursor-pointer text-gray-400 hover:text-gray-300" onclick={PageService.back}>
		Back
	</p>
</div>
