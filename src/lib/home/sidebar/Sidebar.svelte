...
function onMouseLeave(
    ...
    if (!isMouseInsideArea ||
        ...
        (type === SidebarType.Left && e.clientX <= 0)) return;
    animationClass = type === SidebarType.Right ? "animate__fadeOutRight" : "animate__fadeOutLeft";
}

function onAnimationEnd() {
    if ((type === SidebarType.Right && animationClass != "animate__fadeOutRight") ||
        (type === SidebarType.Left && animationClass != "animate__fadeOutLeft")) return;
...
<div
        class={`fixed ${type === SidebarType.Right ? 'right-0' : 'left-0'} top-0 z-10 h-[calc(100%-8rem)] w-full sm:w-[70%] md:w-[50%] hdpi:w-[40%] xhdpi:w-[35%] lg:w-[50%] xl:w-[25%] px-3
...
>
    ...
    >
        <div class="border border-gray-400 bg-white/5 h-full w-full rounded flex flex-col">
            {@render children?.()}
        </div>
    </div>
</div>
