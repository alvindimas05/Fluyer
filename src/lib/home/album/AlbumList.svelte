...
function groupByAlbum(): MusicData[][] {
    const musicData = MusicController.musicList!;
    ...
    return Object.keys(albumsMap)
        .sort()
        .map(key => MusicController.sortMusicList(albumsMap[key]));
}
...
let data: any[] = $derived.by(() => {
    ...
    if (search || $filterAlbum) {
        list = list.filter((musicList) => {
            ...
            return (
                ($filterAlbum && firstItem.album === $filterAlbum.name) ||
    ...
    if (!$filterBarSortAsc) list = list.toReversed();

    return $musicListType === MusicListType.Album
...
const unsubscribeMusicList = musicList.subscribe(() =>
    MusicController.setMusicAlbumList(groupByAlbum())
);

const unsubscribeScrollIndex = albumListScrollIndex.subscribe((value) => {
    if (value >= 0) {
        scrollTo(value);
