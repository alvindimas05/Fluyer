...
const data = $derived.by(() => {
	...
	const filteredMusic = LibraryService.sortMusicList(
		musicStore.list.filter((music) => {
			...
			const matchesFolder = FolderService.containsMusic(music, folderStore.currentFolder);

			if (isFolderMode) return matchesFolder && (!hasSearch || matchesSearch);

			if (!hasAlbum) return !hasSearch || matchesSearch;

			return matchesAlbum && (!hasSearch || matchesSearch);
		})
	);

	let filteredFolders = folderStore.list.filter((f) => f.path.toLowerCase().includes(search));
