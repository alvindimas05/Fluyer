...
<div class="relative text-sm md:text-base">
    <div class="grid grid-cols-[max-content_auto_max-content] py-2">
        ...
        {:then image}
            {#if image}
                <img
                    class="w-12 h-12 md:w-14 md:h-14 object-cover relative rounded {shouldAnimate &&
            ...
            {:else}
                <div
                    class="w-12 h-12 md:w-14 md:h-14 relative aspect-square"
                ></div>
            {/if}
        {/await}
        ...
        <div class="ms-3 overflow-hidden">
            ...
            <p
                class="text-opacity-background-90 whitespace-nowrap overflow-hidden text-xs/[14px] pt-[4px] md:text-xs md:pt-0 animate-scroll-overflow-text"
            >
                {vm.mediumLabel}
            </p>
            <p
                class="text-xs/[14px] md:text-xs mt-[2px] text-opacity-background-90"
            >
                {vm.smallLabel}
            </p>
        </div>
    ...
    <div class="absolute top-0 left-0 py-2 w-full">
        <div
            class="w-full grid grid-cols-[max-content_auto_max-content] music-item-play"
        >
            <button
            ...
            <div
                class={folder ? "cursor-pointer" : "cursor-default"}
                onclick={vm.selectFolder}
            ></div>
