...
<div class="h-fit px-3 pb-3 row-[1] col-auto">
    <div class="relative w-full">
        ...
        {:then image}
            {#if image}
                <img
                    class="rounded-lg w-full aspect-square object-cover {isDesktop() &&
                        !isLinux() &&
                ...
                />
            {:else}
                <div class="rounded-lg w-full aspect-square"></div>
            {/if}
        {/await}
    </div>
    <p
        class="font-medium md:text-lg mt-2 whitespace-nowrap overflow-hidden animate-scroll-overflow-text"
    >
        {vm.music.album}
    </p>
    <p
        class="text-[15px] md:text-base text-opacity-background-80 whitespace-nowrap overflow-hidden animate-scroll-overflow-text"
    >
