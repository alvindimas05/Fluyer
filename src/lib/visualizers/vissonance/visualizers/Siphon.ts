...
class Siphon extends Visualizer {
	...
	render() {
		...
		for (var c = 0; c < this.group.children.length; c++) {
			this.group.children[c].position.z +=
				loudness <= 1
					? 0
					: (Math.pow(loudness / 8192 + 1, 2) - 1) * loudness * 4;
	...
	scaleGroupVectorLength(groupC: number, groupI: number, length: number) {
		var v3 = new THREE.Vector3(
			this.group.children[groupC].geometry.attributes.position.array[
				groupI + 0
			],
			this.group.children[groupC].geometry.attributes.position.array[
				groupI + 1
		...
		var scalar =
			(length + this.cylRadius) / v3.distanceTo(new THREE.Vector3(0, 0, 0));
