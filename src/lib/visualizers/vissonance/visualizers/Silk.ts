...
class Barred extends Visualizer {
    ...
    async make() {
        ...
        View.data.renderer = new THREE.WebGLRenderer( { alpha: true, preserveDrawingBuffer: true } );
    ...
    render() {
        ...
        for(let i = 0; i < visualArray.length; i++) {
            this.group.children[i].children[0].scale.x = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
            this.group.children[i].children[0].scale.y = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group2.children[i].children[0].scale.x = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
            this.group2.children[i].children[0].scale.y = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group3.children[i].children[0].scale.x = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
            this.group3.children[i].children[0].scale.y = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group4.children[i].children[0].scale.x = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
            this.group4.children[i].children[0].scale.y = (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
            //

            this.group.children[i].position.y += (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group2.children[i].position.y += (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group3.children[i].position.y -= (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );

            this.group4.children[i].position.y -= (visualArray[i] <= 1) ? 0.0001 : 4 * Math.log10( 1 + (visualArray[i] / 255 / 7 ) );
    ...
    setUniformColor(groupI: number, loudness: number) {
        ...
        this.group.children[groupI].children[0].material.uniforms.col.value = new THREE.Color( 'hsl(' + h + ', 90%, ' + ( 100 - Math.min(40, loudness ) ) + '%)' );
    }
}
