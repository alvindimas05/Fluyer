...
const Spectrum = {
    getVisualBins: (dataArray: Uint8Array, numElements: number, SpectrumStart: number, SpectrumEnd: number) => {
        const SpectrumBarCount = numElements;
        let SamplePoints = [];
        let NewArray = [];
        let LastSpot = 0;
        for(let i = 0; i < SpectrumBarCount; i++ ) {
            let Bin = Math.round( Spectrum.ease( i / SpectrumBarCount ) * ( SpectrumEnd - SpectrumStart ) + SpectrumStart );
            if( Bin <= LastSpot ) {
                Bin = LastSpot + 1;
            }
            LastSpot = Bin;
    ...
    exponentialTransform: (array: number[]) => {
        ...
        for (let i = 0; i < array.length; i++) {
            const exp = spectrumMaxExponent + (spectrumMinExponent - spectrumMaxExponent) * (i / array.length);
